...
import { codingWords } from "./index.components";
    ...
    <div className="w-full touch-none z-[999999] h-svh fixed inset-0 flex justify-center">
      ...
      <div className="relative top-[6rem]">
        ...
        <div className="flex w-full">
          <ul className="cursor-pointer dark:text-text-dark-primary text-text-light-primary mt-2 border-1 rounded-md dark:border-border-dark border-border-light w-full bg-bg-light-secondary dark:bg-bg-dark-secondary max-h-[25rem] overflow-y-auto scrollbar">
            {userSearch.trim() === "" ? (
              ...
              <div>
                {result.map((word, id) => {
                  return (
                    <li
                      className="mx-2 border-b-[0.5px] dark:border-border-dark border-border-light flex items-center px-2 py-1.5"
                      key={id}
                    >
                      {word}
                    </li>
